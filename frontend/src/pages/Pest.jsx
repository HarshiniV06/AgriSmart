...
export default function Pest() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post("/predict-pest", formData);
      setResult(res.data);
    } catch (err) {
      console.error("Pest request error:", err);
      const status = err.response?.status;
      const serverMsg = err.response?.data || err.message;
      setError(
        status ? `Error ${status}: ${JSON.stringify(serverMsg)}` : String(serverMsg || "Pest detection failed")
      );
    }
    setLoading(false);
  };
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center p-10">
        <div className="bg-white p-8 rounded-xl shadow-xl w-full max-w-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {loading ? t('common.loading') : t('modules.pest.submit')}
            </button>
          </form>
          ...
          {result && (
            result.message ? (
              ...
              <ResultCard
                ...
                subtitle={result.pest}
                main={<span className="text-green-700 font-bold">{result.pest}</span>}
